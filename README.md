...
## 검색 자동완성
상단 검색창에 글자를 입력하면 이름에 해당 글자가 포함된 가게가 실시간으로 표시됩니다.
(예: "금" → 금수복국 등) — `/api/stores/suggest` 호출

## 부미로(釜味路)의 특색 기능
- **지역별 맛집 코스**: 지역을 선택하면 아침(카페)/점심(식사류)/저녁(식사류, 카페 제외) 코스를 자동 추천.
  - 선정 순위: 리뷰 10개 이상 별점순 → 리뷰 많은순 → 프랜차이즈 제외 로컬 우선 (콜드스타트 대응)
- **코스 맛집 리뷰 2배 포인트**: 코스에 포함된 가게에 리뷰를 쓰면 200P 적립 (일반 100P).
  - 코스대로 다녀보도록 유도하는 리워드 연계.
